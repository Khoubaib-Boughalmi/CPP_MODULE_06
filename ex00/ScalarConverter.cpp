...
void convertChar(std::string input) {
    ...
    if(detectScalarType(input) == CHAR_TYPE)
        std::cout << "Char: " << GREEN << "'" << input << "'" << RESET << std::endl;
    else
    {
...
void convertDecimal(std::string input) {
    ...
    if(errno == ERANGE)
        throw ScalarConverter::ErrorExeption();
    if(detectScalarType(input) == CHAR_TYPE)
        std::cout << "Decimal: " << GREEN << static_cast<int>(input[0]) << RESET << std::endl;
    else
...
void convertFloat(std::string input) {
    ...
    if(errno == ERANGE)
        throw ScalarConverter::ErrorExeption();
    if(detectScalarType(input) == CHAR_TYPE)
        std::cout << "Float: " << GREEN << static_cast<int>(input[0]) << "f" << RESET << std::endl;
    else
    {
...
void convertDouble(std::string input) {
    ...
    if(errno == ERANGE)
        throw ScalarConverter::ErrorExeption();
    if(detectScalarType(input) == CHAR_TYPE)
        std::cout << "Double: " << GREEN << static_cast<int>(input[0]) << RESET << std::endl;
    else
    {
        if(result - static_cast<int>(result) == 0)
...
void ScalarConverter::convert(std::string input) {
    scalarTypes scalarType = detectScalarType(input);
    displayConversion(scalarType, input);
}